...
function compareTriplets(a, b) {
    ...
    const aliceBase = a.map((valueA) => { return { aliceValue: valueA } })
    const bobBase = b.map((valueB) => { return { bobValue: valueB } })

    let resultAlice = []
    let resultBob = []

    if(aliceBase[0].aliceValue > bobBase[0].bobValue) { const alicePush0 = resultAlice.push(1) }
    ...
    if(aliceBase.aliceValue === bobBase.bobValue) { const alicePush = resultAlice.push(0) }
    if(aliceBase.aliceValue === bobBase.bobValue){ const bobPush = resultBob.push(0) }

    else { resultAlice.push(0) && resultBob.push(0) }
